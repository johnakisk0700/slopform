...
| Add persistence      | Explicit queries in a domain repository. PostgreSQL (including feedback conversations): Drizzle schemas/migrations. MongoDB Assistant threads: versioned Zod documents + reviewed indexes. Expose neither client to controllers.                                                                                               |
...
| Add a queue job      | Versioned name, identifier-only envelope, typed producer envelope, validate at the consuming boundary, deliberate retry/retention; update `docs/backend/mechanisms/queues.md`.                                                                                                                                                 |
| Add a worker         | Processors only in a domain worker module imported by `worker-app.module.ts`; keep HTTP providers out of the worker graph.                                                                                                                                                                                                     |
...
| Change logging       | `infrastructure/logging/`; keep redaction, unexpected-error logging and startup cleanup; nginx owns access logs and Docker owns application rotation; update the runtime mechanism page.                                                                                                                                       |
...
| Add a test           | Co-locate focused behavioral `*.spec.ts`; custom scenarios, doubles and rehearsal tools belong in [harness](../../harness/README.md). HTTP routes build the real app (see "Environment and operation" in `docs/backend.md`). Adapter semantics: real PostgreSQL/Redis with bounded exact cleanup, not mocked fluent internals. |

Validate incoming HTTP requests, consumed queue payloads and untyped external or
