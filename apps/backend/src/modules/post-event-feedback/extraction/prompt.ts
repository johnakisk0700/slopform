...
function buildSystemPrompt(context: FeedbackExtractionContext): string {
  ...
  return [
    ...
    "1β. Το ΠΛΑΙΣΙΟ ΧΩΡΟΥ είναι fallible πληροφορία που επιβεβαίωσε χειριστής, όχι μαρτυρία του participant. Δεν τεκμηριώνει ΠΟΤΕ answer, note, παραγγελία, πράξη ή γνώμη και δεν υπερισχύει από όσα έγραψε ο άνθρωπος. Αν διαφωνούν, κρατάς τα λόγια του ανθρώπου ακριβώς όπως είναι· δεν τα διορθώνεις για να ταιριάξουν στον χώρο.",
    ...
    context.goals.some((goal) => goal.key === "liked")
      ...
      : "7. Στο goals απαντάς για ΚΑΘΕ στόχο, πάντα, με ένα από τέσσερα status. Δεν παραλείπεις στόχο: αν δεν τον άγγιξε το μήνυμα, το λες με not_addressed. answered = τα νέα μηνύματα τον απαντούν, και στο answers βάζεις ΟΛΕΣ τις απαντήσεις αυτού του στόχου (π.χ. «θα ξαναέβλεπα τον Νίκο και την Ελένη» είναι δύο απαντήσεις στο meet_again). declined = ο συμμετέχων αρνήθηκε ρητά ή είπε ότι δεν έχει κάτι να πει (π.χ. «κανέναν», «όλοι καλοί ήταν»). already_settled = ήταν ήδη απαντημένος ή προσπερασμένος πριν από αυτή την κλήση.",
    context.goals.some((goal) => goal.key === "liked")
      ...
      : "7β. Δουλεύεις στόχο-προς-στόχο, όχι φράση-προς-φράση: για κάθε στόχο ξεχωριστά διάβασε ΟΛΟΚΛΗΡΟ το μήνυμα και μετά αποφάσισε το status του. Οι στόχοι δεν αποκλείουν ο ένας τον άλλον. Μια φράση μπορεί να απαντά σε πολλούς στόχους μαζί: «η βραδιά ήταν 5, η παρέα μου ταίριαξε 4 και θα ξαναέβλεπα τη Μαρία» απαντά στα event_score, table_fit και meet_again, και γράφεις και τα τρία. Το ίδιο ισχύει για το declined: «κανέναν να αποφύγω» κλείνει το avoid ακόμα κι όταν η ίδια φράση απάντησε και άλλον στόχο.",
    ...
    "11θ. Αν σε ρωτήσουν αν είσαι άνθρωπος ή μηχανή, ΠΟΤΕ μην ισχυριστείς ότι είσαι άνθρωπος και μην το αφήσεις να εννοηθεί — ούτε για πλάκα, ούτε για να κυλήσει πιο ζεστά η κουβέντα. Πες το απλά και προχώρα· δεν χρειάζεται απολογία ούτε τεχνικές λεπτομέρειες.",
    "11ι. Το ΠΛΑΙΣΙΟ ΧΩΡΟΥ μπορεί να βοηθήσει μόνο το reply να ακούγεται σχετικό. Αν υπάρχει καθαρή, ακίνδυνη αναντιστοιχία — π.χ. ιαπωνικό εστιατόριο και ο participant μιλά για ιταλική πίτσα — μπορείς να την παρατηρήσεις απαλά ή να κάνεις ΕΝΑ σύντομο, γενικό και προφανώς φανταστικό αστείο. Δεν κοροϊδεύεις τον participant ή το τραπέζι, δεν λες τι υποτίθεται ότι έφαγε ή έκανε και δεν μετατρέπεις το αστείο σε answer/note. Σε παράπονο, ένταση, safety θέμα ή κακή μεταχείριση ο 11γ υπερισχύει και δεν αστειεύεσαι.",
...
function buildUserPrompt(input: BuildFeedbackExtractionPromptInput): string {
  ...
  return [
    "ΕΡΩΤΗΣΕΙΣ ΚΑΜΠΑΝΙΑΣ",
    formatQuestions(context),
    ...(context.venue
      ? [
          "",
          "ΠΛΑΙΣΙΟ ΧΩΡΟΥ (χειριστή· όχι μαρτυρία)",
          formatVenueContext(context.venue),
        ]
      : []),
    "",
    "ΣΤΟΧΟΙ",
...
function formatVenueContext(
  ...
  if (venue.area) {
    lines.push(`- περιοχή: ${JSON.stringify(venue.area)}`);
  }
  if (venue.priceRange) {
    const start = formatMinorAmount(
      venue.priceRange.startMinor,
      venue.priceRange.currencyCode,
    );
    const end =
      venue.priceRange.endMinor === undefined
        ? null
        : formatMinorAmount(
            venue.priceRange.endMinor,
            venue.priceRange.currencyCode,
          );
    lines.push(
      `- κόστος ανά άτομο: ${start}${end ? `–${end}` : ""} ${venue.priceRange.currencyCode}`,
    );
  } else if (venue.priceLevel) {
    lines.push(`- επίπεδο κόστους: ${venue.priceLevel.replaceAll("_", " ")}`);
  }
  return lines.join("\n");
}

function formatMinorAmount(minor: number, currencyCode: string): string {
  const digits =
  ...
  return (minor / 10 ** digits)
    .toFixed(digits)
    .replace(/(?:\.0+|(?<fraction>\.\d*?[1-9])0+)$/u, "$<fraction>");
}

function formatQuestions(context: FeedbackExtractionContext): string {
  return context.goals
    .map((goal) => {
      const question = getPostEventFeedbackAnswerQuestionDefinition(goal.key);
      if (!question) {
        throw new Error(`Unknown feedback question key: ${goal.key}`);
      }
      const shape =
      ...
      return `- ${question.key} (${shape}): ${goal.prompt}`;
    })
    .join("\n");
}
