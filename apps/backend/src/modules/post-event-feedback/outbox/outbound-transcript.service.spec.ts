...
describe("FeedbackOutboundTranscriptService", () => {
  ...
  it.each([
    ...
    async (kind, actor) => {
      ...
      await expect(
        service.record(transaction, outboxRow({ kind }), at),
      ).resolves.toEqual({
        outcome: "appended",
  ...
  it("cancels the row instead of sending a message it cannot record", async () => {
    ...
    await expect(
      service.record(transaction, outboxRow(), at, "corr-1"),
    ).resolves.toEqual({
      outcome: "cancelled",
  ...
  it("cancels and flags a body the transcript cannot hold, without retrying forever", async () => {
    ...
    await expect(
      service.record(transaction, outboxRow({ body }), at),
    ).resolves.toEqual({
      outcome: "cancelled",
  ...
  it("propagates a non-capacity persistence failure instead of cancelling", async () => {
    const { service, conversations, repository } = createService();
    conversations.appendMessage.mockRejectedValue(new Error("storage down"));

    await expect(service.record(transaction, outboxRow(), at)).rejects.toThrow(
      "storage down",
    );
    expect(repository.updateOutboxStatus).not.toHaveBeenCalled();
  });
