...
| Secondary action | No separate `--jts-color-secondary` token | HeroUI `variant="secondary"` | A lower emphasis action. HeroUI maps it to the default surface (`--jts-color-surface-sunken`) with primary readable text. Use the component variant; do not invent a second brand colour. |
...
`jts-title-mark`; the sidebar index is the navigation indicator.
...
For the full token graph and palette accessibility floors, see
[`docs/frontend/theming.md`](../../docs/frontend/theming.md). For ownership
and API/runtime rules, see [`AGENTS.md`](./AGENTS.md).
