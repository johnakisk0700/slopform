...
flowchart LR
  ...
  Worker --> Mongo[(MongoDB Assistant)]
...
| Topic                    | Rule                                                                                                                                                                                                                                                                                                                           |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Wake-ups                 | Disposable. PostgreSQL stores `{ work_revision, work_next_action_at }`; schedule increments revision and commits before `Queue.add`. Missing Redis job delays until maintenance republishes the same revision. Epoch stays on the fence table.                                                                                 |
| Reconcile                | Seven-minute PG lease on `feedback_conversation_executions`; admission checks conversation revision in the same short transaction as `tryClaim`; reloads state; at most one action. Settlement writes successor due time only if this revision still owns the row.                                                             |
...
| Provider entry           | After limiter grants a slot: short PG txn takes ingress phone + conversation advisory locks, validates execution token, share-locks campaign, locks consent, rejects durable inbound beyond the conversation snapshot, final conversation revision/lifecycle/control read; commits **before** network call (billing boundary). |
| Extraction write         | After the model returns, answers, notes, audit, outbox, transcript, goals, attention and cursor commit in one service-owned transaction. Operator alerts and BullMQ publication stay after commit.                                                                                                                             |
...
503 (row stays `pending`). Worker: append or STOP on the conversation row, then
mark the ingress terminal. Session advisory locks may still serialize a drain
of several ingress rows. Maintenance recovers `pending` older than
...
(`removeOnComplete: true`). Terminal extraction fallback clears
`work_next_action_at` without advancing revision so the failed wake-up stays
under failed retention.
