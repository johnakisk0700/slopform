...
DataTable and Toolbar are local imports because the reference consumer is in
the client-rendered admin boundary. Add them to the PrimeVue Nuxt allow-list
before rendering this component during SSR. Add sorting, selection or lazy
pagination inputs only when a second real table requires a common contract.
