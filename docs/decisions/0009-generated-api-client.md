...
- `apps/admin/src/features/auth/schema.ts` is deleted; `RequireAdmin` uses
  `useGetAuthSession`. The assistant polling flow keeps its hand-written schemas
  until its own work package migrates it, because it owns client-side semantics
  beyond the response shape.
