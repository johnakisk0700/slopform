...
| `assertCleanSimulatorBaseline` (was WP-22)                                                   | The preflight and write-time checks read the same invariants and say different things on purpose. Preflight reports a dirty baseline; write-time reports that a baseline which passed preflight _changed underneath the run_ — a race diagnosis the operator needs and a merged version would lose. Intro repair also sits between the write-time checks, and the message-limit math differs (`effectiveMessageCount` vs post-repair length). Unifying them changes thrown messages and control flow in a dev-only simulator: SIMPLIFY dressed as dedupe. Two guards that resemble each other are not a copy. |
...
**WP-22 — withdrawn.** See Rejected. The two simulator baseline checks are not one body with two
call sites, so there is nothing here to extract.
...
Files: `infrastructure/config/environment.ts` (581 → ~380) → new `mongo-connection-string.ts` (L6-104 parser + the production URI rules at L487-533, which become an exported **`addProductionMongoIssues`** taking the parsed connection string and the Zod refinement context and adding the three issues it adds today — missing credentials, disabled TLS verification, and no TLS outside the internal `mongo` service. The `NODE_ENV === "production"` test stays at the call site, so the function is about the URI, not about the environment. Name added 2026-07-26 after the first dispatch correctly refused to invent it.), new `enabled-modules.ts` (L544-581, the five `is*Enabled(NodeJS.ProcessEnv)` readers), new `environment-values.ts` (`emptyStringToUndefined` + `parseUrl`, currently duplicated in `observability-environment.ts:3`).
...
imports. The real deletions — the duplicated fakes (WP-16), seven copies of `errorMessage` (WP-07), three of
four copy resolvers (WP-12) — total well under 1,000 lines against 63,641, and less than first
estimated now that WP-22's ~130 lines turned out not to be duplication at all. Judge this campaign on placement, not on volume.
